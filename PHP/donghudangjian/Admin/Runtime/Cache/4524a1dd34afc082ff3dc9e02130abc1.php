...
<body>
<form name='myform' id='myform' action="<?php echo U('Type/save_index_display_manage');?>" method='post'>
...
<tr> 
 ...
 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr bgcolor="#f1f3f5" datatype='volist'  onMouseOver="this.style.backgroundColor='#EAFCD5';this.style.color='red'" onMouseOut="this.style.backgroundColor='';this.style.color=''" class='none' id="<?php echo ($vo["bpath"]); ?>" path='<?php echo ($vo["path"]); ?>'>
  ...
  <td>
    <input type="checkbox" name="id[]" value="<?php echo ($vo["id"]); ?>" checked style="display: none"/>
    <input type="checkbox" name="is_index_main_display[]"  <?php if(($vo["is_index_main_display"]) == "1"): ?>checked<?php endif; ?> value="<?php echo ($vo["id"]); ?>"/>
  </td>
  <td width="10%"><input type='text' name='sortrank_index_main_display[]' value='<?php echo ($vo["sortrank_index_main_display"]); ?>' 
         style='width:30px;'/></td>
