...
class UpdateAction extends CommonAction
{
	public function index()
	{
		if($this->isPost())
		{
			...
			$msg = fopen_url('http://www.waikucms.com/update.php?version='.$version);
			$data['version'] = fopen_url('http://www.waikucms.com/update.php?fullversion=1');
	...
	{	
		...
		$msg = fopen_url('http://www.waikucms.com/update.php?version='.substr(C('SOFT_VERSION'),-8));
		$msg = $msg<>0 && $msg<>1 ? 2:$msg; 
		//$msg=1;
		if($msg==0) $this->error('当前系统已经是最新版!');
		$nowversion = fopen_url('http://www.waikucms.com/update.php?fullversion=1');
		if($msg==2) $this->error('更新检测失败!');
		$updateurl = fopen_url('http://www.waikucms.com/update.php?updateurl=1');
