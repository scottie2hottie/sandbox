		...
		if($tag['group'] == $mode)
		{
			...
			elseif($tag['tag']=='checkbox')
			{
				...
				foreach($values as $k=>$v)
				{
					...
					$tagvar.=$v."<input name='{$tag['name']}-{$k}' group='{$mode}' type='checkbox' class='noborder' id='{$tag['name']}-{$k}' value='{$k}' style='{$tag['style']}' {$tag['single']}/>";
				}
			}
			...
			elseif($tag['tag']=='editor')
			{
				...
				$tagvar .="<textarea name='{$tag['name']}' id='{$tag['id']}' group='{$mode}' cols='60' rows='60' style='{$tag['style']}' >{$tag['value']}</textarea>";
			}
			elseif($tag['tag']=='radio')
			{
				...
				{
					$tag['single']= in_array($k,$checked) ? 'checked' : '';
					$tagvar.=$v."<input name='{$tag['name']}' type='radio' group='{$mode}' class='noborder' id='{$tag['name']}-{$k}' value='{$k}' style='{$tag['style']}' {$tag['single']}/>";
				}
			}
