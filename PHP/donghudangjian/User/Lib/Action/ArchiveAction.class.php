...
class ArchiveAction extends CommonAction
{
   ...
   protected function parsefieldset($fieldset,$mode='1')
   {
	...
	private function parsefieldsetmode($tag,$mode,$display=false)
	{
		...
		if($tag['group'] == $mode)
		{
			if($display==true) $tag['value'] = "<field name='{$tag['name']}' prefix='addonlist'/>";
			$tag_checked =substr(strrchr($tag['single'],'#'),1);
			$checked = empty($tag_checked) ? array(99) : explode(",",$tag_checked);
			if($tag['tag']=='input')
			{
				$tagvar = "<input type='{$tag['type']}' group='{$mode}' name='{$tag['name']}' id='{$tag['id']}' size='{$tag['size']}'  style='{$tag['style']}' value='{$tag['value']}' {$tag['single']}/>";
			}
			elseif($tag['tag']=='checkbox')
			{
				...
				foreach($values as $k=>$v)
				{
					$tag['single']= in_array($k,$checked) ? 'checked' : '';
					
					$tagvar.=$v."<input name='{$tag['name']}-{$k}' group='{$mode}' type='checkbox' id='{$tag['name']}-{$k}' value='{$k}' style='{$tag['style']}' {$tag['single']}/>";
				}
			}
			elseif($tag['tag']=='textarea')
			{
				$tagvar = "<textarea name='{$tag['name']}' group='{$mode}'  id='{$tag['id']}' cols='60' rows='60' style='{$tag['style']}' >{$tag['value']}</textarea>";
			}
			elseif($tag['tag']=='editor')
			{
				$tagvar .= "<script charset='utf-8' src='".__ROOT__."/Public/Editor/kindeditor/editor.php?theme=simple&fm=false&id=Content'></script>";
				$tagvar .="<textarea name='{$tag['name']}' id='Content' group='{$mode}' cols='60' rows='60' style='width:100%;height:300px;' >{$tag['value']}</textarea>";
			}
			elseif($tag['tag']=='radio')
			{
				...
				{
					$tag['single']= in_array($k,$checked) ? 'checked' : '';
					$tagvar.=$v."<input name='{$tag['name']}' type='radio' group='{$mode}' id='{$tag['name']}-{$k}' value='{$k}' style='{$tag['style']}' {$tag['single']}/>";
				}
			}
			elseif($tag['tag']=='select')
			{
				$tagvar .= "<select name='{$tag['name']}' group='{$mode}' style='{$tag['style']}'>";
				$values = explode(",",$tag['value']);
				foreach($values as $k=>$v)
				{
					$tag['single']= in_array($k,$checked) ? "selected='selected'" : '';
					$tagvar.="<option value='{$k}' {$tag['single']}>{$v}</option>";
				}
				$tagvar .= "</select>";
			}
			return $tagvar;
