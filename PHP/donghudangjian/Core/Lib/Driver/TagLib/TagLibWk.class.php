...
class TagLibWk extends TagLib {
    ...
    protected $tags   =  array(
		...
		'arclist'	 =>	 array('attr'=>'vo,fromvo,flag,noflag,field,typeid,modelid,row,titlelen,subday,orderby,limit,key,mod,aid,keyword,arcrank,noarcrank,getall,pagesize,paging,noglobal,likearticle','level'=>3),
		...
		'arctype'	 =>	 array('attr'=>'vo,fromvo,typeid,row,limit,field,type,modeltype,modelid,key,mod','level'=>3),
		'page'       =>  array('attr'=>'linkpage,option','close'=>0),
		'myad'       =>  array('attr'=>'id,gid,orderby,limit,row,key,mod','level'=>3),
		'include'    =>  array('attr'=>'filename','close'=>0),
		'plugin'    =>  array('attr'=>'name,method,parameter,function','close'=>0),
		'memberlist'    =>  array('attr'=>'field,id,money,row,limit,rankid,orderby,status,key,mod','level'=>3),
		'flink'    =>  array('attr'=>'id,gid,img,row,limit,orderby,key,mod','level'=>3),
		'vote'    => array('attr'=>'id','close'=>0),
		'votelist' => array('attr'=>'field,titlelen,id,orderby,row,limit,fromvo,vo,key,mod,type,option','level'=>3),
		'taglist' =>array('attr'=>'field,titlelen,orderby,row,limit,fromvo,vo,key,mod,subday','level'=>3),
     ...
     */
	 ...
	 public function _page($attr)
	 {
		...
		$parseStr 		= ''; 
		$parseStr  	   .= '<?php $__PAGE__ = _page_output($totalnum,\''.$_fields['page']['pagesize'].'\',\''.$linknum.'\',\''.$option.'\');';
     ...
     */
	 ...
	 public function _arctype($attr,$content)
	 {
		...
		$row        	 =    isset($tag['row']) ? (int)$tag['row'] : 10;
		$limit        	 =    isset($tag['limit']) ? $tag['limit'] : '';
		$modeltype    	 =    isset($tag['modeltype']) ? (int)$tag['modeltype'] : '';
		$key  		 	 =    !empty($tag['key'])?$tag['key']:'i';
		$mod  		 	 =    isset($tag['mod'])?$tag['mod']:'2';
		$vo			  	 =	  isset($tag['vo']) ? $tag['vo']:'';
		$fromvo		     =	  isset($tag['fromvo']) ? $tag['fromvo']:'';
		...
		//栏目属性modeltype 
		if(!empty($modeltype) && $modeltype<=2) $map['modeltype'] = array('in',$modeltype);
		$root  = $GLOBALS['cfg_rewrite']==0 ? __ROOT__ :__ROOT__.'/index.php';
		$suffix = '.'.C('URL_HTML_SUFFIX');
		if(!empty($field)) $field = rtrim($field,',').',';
