...
class CommonAction extends Action
{
	function _initialize() 
	{
		...
		$map['is_index_menu_display'] = 1;
		$rs_nav = $arctype_table->where($map)->select();
		$this->assign('nav', $rs_nav);

		//ad-head-468x60
		$ad_table = M("Ad");
		$map['id'] = 5;
		$rs_ad_head_468x60 = $ad_table->where($map)->field('content')->find();
		$this->assign('ad_head_468x60', $rs_ad_head_468x60);
	}
	//位置导航
