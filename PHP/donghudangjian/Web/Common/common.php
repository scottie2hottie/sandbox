...
*************************************************************/
	...
	*/
	function _arclist_getall_sonid($typeid)
	{
		...
		$data['path'][] = array('like','%'.$bpath['bpath']);
		$data['path'][] = 'or';
	...
	*/
	...
	/*
		page 标签  动态输出 分页信息
	*/
	function _page_output($totalnum,$pagesize=20,$linknum=5,$option)
	{
		...
		$request_uri = isset($_GET['p']) ? strtr($_SERVER['REQUEST_URI'],array('&p='.$_GET['p']=>'','?p='.$_GET['p']=>'')) : $_SERVER['REQUEST_URI'];
		$Delimiter  = strpos($request_uri,'?')!==false ? '&' : '?';
		$pageurl = $request_uri.$Delimiter.'p=';
